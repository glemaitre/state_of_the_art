...
\begin{equation}
	p(C=k,\mathbf{x}) = \frac{p(k)p(\mathbf{x}|k)}{p(\mathbf{x})} \ .
	\label{eq:bayth}
\end{equation}

\noindent where $p(C=k,\mathbf{x})$ is the posterior probability, $p(k)$ is the prior probability, $p(\mathbf{x},k)$ is the likelihood and $p(k)$ is the evidence. 

However, the evidence term is usually discarded since, it is not class dependent and play the role of a normalization term. Hence, in a classification scheme, an unlabelled observation will be classified to the class which maximized the posterior probability such as:

\begin{eqnarray}
	C(\mathbf{x}_i) & = & \argmax_k p(C=k|\mathbf{x}_i) \ , \label{eq:maxbay} \\
	p(C=k|\mathbf{x}_i) & = & p(k) \prod_{j=1}^{n} p(x_{ij},|k) \ . \label{eq:postbay}
\end{eqnarray}

\noindent where $n$ is the number of dimension of the feature vector $\mathbf{x}_i = \{x_{i1},\cdots,x_{in}\}$.

Usually, a model have to be affected for the prior and likelihood probabilities and it is common to affect an equal prior probability for each class or eventually a value depending of the relative frequency derived from the training set. Regarding the likelihood probability, it is common to choose a Normal distribution to characterized each class. Thus, each class will be characterized by two parameters: (i) the mean and (ii) the standard deviation. These parameters can be inferred from the training set by using the maximum likelihood approach.
...
In the prediction stage, a sample is introduced in each tree and each of the tree will assign a class to this sample. Finally, it is common to use a a majority voting approach to choose the final class labelling.
...
\noindent where $\phi(.)$ is a Gaussian basis function, $\mathbf{x}_n|n=\{1,\cdots,N\}$ are the $N$ training points and $\boldsymbol{\alpha}$ are the weights.
...
\begin{tikzpicture}[shorten >=1pt,draw=black!50, node distance=\layersep]
   ...
   \node[hidden neuron] (H-1) at (\layersep,0 cm) {\tiny $h_{11}\left(.\right)$};
   \node[hidden neuron] (H-2) at (\layersep,-1 cm) {\tiny $h_{21}\left(.\right)$};
   \node[hidden neuron] (H-3) at (\layersep,-2.5 cm) {\tiny $h_{31}\left(.\right)$};
   ...
   \node[hidden neuron] (H-4) at (\layersep,-4.5 cm) {\tiny $h_{12}\left(.\right)$};
   \node[hidden neuron] (H-5) at (\layersep,-5.5 cm) {\tiny $h_{22}\left(.\right)$};
   \node[hidden neuron] (H-6) at (\layersep,-7 cm) {\tiny $h_{32}\left(.\right)$};
