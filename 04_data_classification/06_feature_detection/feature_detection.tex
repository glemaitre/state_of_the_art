...
\begin{table*}
	...
	\begin{tabular}{p{.6\linewidth} p{.3\linewidth}}
		...
		\quad \quad \quad Wavelet-based features & $[$37$]^{\text{{\cmarksmall}- -}}$ \\
		\quad \quad Position-based & $[$6,10-11$]$ \\ \\ [-1.5ex]
		\quad \textit{Region-wise detection} &  \\ \\ [-1.5ex]
		\quad \quad Statistical-based & \\
		\quad \quad \quad Percentiles & $[$39$]^{\text{- {\cmarksmall}-}}$ \par $[$3,23$]^{\text{- - {\cmarksmall}}}$\par $[$40$]^{\text{{\cmarksmall}{\cmarksmall}-}}$\par $[$10-11,18-19,41$]^{\text{{\cmarksmall}{\cmarksmall}{\cmarksmall}}}$ \\
		\quad \quad \quad Statistical-moments & $[$1-2,29-30,32-33,35,37$]^{\text{{\cmarksmall}- -}}$\par $[$3$]^{\text{- - {\cmarksmall}}}$\par $[$36$]^{\text{{\cmarksmall}{\cmarksmall}-}}$\par $[$23$]^{\text{{\cmarksmall}- {\cmarksmall}}}$\par $[$10-11,18-19$]^{\text{{\cmarksmall}{\cmarksmall}{\cmarksmall}}}$ \\
		\quad \quad Histogram-based & \\
		\quad \quad \quad \acs{pdf} & $[$13$]^{\text{{\cmarksmall}{\cmarksmall}{\cmarksmall}}}$ \\
		\quad \quad \quad \acs{hog} & $[$13$]^{\text{{\cmarksmall}{\cmarksmall}{\cmarksmall}}}$ \\
		\quad \quad \quad Shape context & $[$13$]^{\text{{\cmarksmall}{\cmarksmall}{\cmarksmall}}}$ \\
		\quad \quad \quad \acs{lbp} & $[$13$]^{\text{{\cmarksmall}{\cmarksmall}{\cmarksmall}}}$ \\
		\quad \quad Anatomical-based & $[$11,15$]$ \\ \\ [-1.5ex]
		\textbf{\ac{dce} signal:} & \\ \\ [-1.5ex]
		\quad Whole spectra approach & $[$1,2$]$ \\
		\quad Semi-quantitative approach & $[$24$]^{\text{{\mmarksmall}}}$\par $[$17-19,25$]$ \\
		\quad Quantitative approach &  \\
		\quad \quad Toft model & $[$13,24$]^{\text{{\mmarksmall}}}$\par $[$7,9-11,17-19$]$ \\
		\quad \quad Brix model & $[$4-5,10-21$]^{\text{{\mmarksmall}}}$\par $[$12,25$]$ \\
		\quad \quad Weibull function & $[$7,17$]$ \\
		\quad \quad PUM & $[$7,17$]$ \\
		\\ [-1.5ex]
		\textbf{\ac{mrsi} signal:} & \\ \\ [-1.5ex]
		\quad Whole spectra approach & $[$8,16,22,26-30,32-33$]$ \\
		\quad Quantification approach & $[$8,22$]$ \\
		\quad Wavelet-based approach & $[$31$]$ \\ \\ [-1.5ex]
		\hline
	\end{tabular}
	\begin{tablenotes}
      \footnotesize
...
The most common approach reviewed can be classified as statiscal methods. Full map corresponding to feature types presented in the previous section are computed. Then, \acp{roi} are defined and statistics are extracted from each of these regions. The first type of statistic is based on percentiles and is widely used (\cite{Antic2013,Litjens2011,Litjens2012,Peng2013,Tiwari2009a,Tiwari2010,Tiwari2013,Viswanath2008,Viswanath2008a,Viswanath2011,Viswanath2012,Vos2008,Vos2008a,Vos2010,Vos2012}). In fact, once that a \ac{roi} is defined, the features corresponding the $n^{\text{th}}$ percentile is used as feature. This $n$ can take any value between $0$ and $100$. This threshold is usually manually determined observing the distribution and corresponds to the best discriminant value differentiating malignant and healthy tissue. in addition, statistic-moments such as mean, standard deviation, kurtosis and skewness are also used (\cite{Ampeliotis2007,Ampeliotis2008,Antic2013,Niaf2011,Niaf2012,Peng2013}).
...
The Tofts model and more precisely the parameters $K_{trans}$, $k_{ep}$ and $v_e$ (\cite{Tofts1997}) was used by \cite{Langer2009,Giannini2013,Niaf2011,Niaf2012,Mazzetti2011}.% The \ac{dce} signal relative to the concentration is modelled as:
%\begin{equation}
